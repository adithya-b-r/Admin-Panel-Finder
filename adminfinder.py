...
def adminFinder():
  url = input(f"{GREEN}[{RED}+{GREEN}]{RESET}{BLUE} Enter website URL: "+RESET).strip()
  totalThread = int(input(f"{GREEN}[{RED}+{GREEN}]{RESET}{BLUE} Enter number of threads: "+RESET))
  ...
  if checkURL(url) == 200:
    ...
    print(f"{GREEN}[{RED}-{GREEN}]{RESET}{RED} Website {url} is not accessible.")

if __name__ == "__main__":
  fileName = input(f"{GREEN}[{RED}+{GREEN}]{RESET}{BLUE} Enter file name: "+RESET)
  adminFinder()